...
const int N = 100000;
const int base[] = {31, 47}, mod[] = {MOD, MOD + 2};
int pwr[2][N + 10], h[2][N + 10];
char str[N + 10];
int n;
ll range_hash(int l, int r){
	int x[2], i;
	for(i = 0; i < 2; ++i){
		x[i] = add(h[i][l], -mul(h[i][r + 1], pwr[i][r - l + 1], mod[i]), mod[i]);
	}
	return ((ll)x[0] << 31ll) | (ll)x[1];
}
void solve(){
	int i, j;
	for(i = 0; i < 2; ++i){
		for(j = 1, pwr[i][0] = 1; j <= n; ++j){
			pwr[i][j] = mul(pwr[i][j - 1], base[i], mod[i]);
		}
	}
	for(i = 0; i < 2; ++i){
		for(j = n - 1; j >= 0; --j){
			h[i][j] = add(str[j] - 'a' + 1, mul(h[i][j + 1], base[i], mod[i]), mod[i]);
		}
	}
	pll(range_hash(0, 2), range_hash(2, 4)); nl;
}
int main(){
