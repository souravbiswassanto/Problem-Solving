...
ll bigmod(ll base,ll exponent){
    ...
    ll r=bigmod(base,exponent/2);
    if(exponent%2){
        return (((r%mod)*(r%mod))%mod*base)%mod;
    }
    else {
...
void divofstring(string s,vector<int>div){
    ll Hash;
    for(int i=0;i<div.size();i++){
        ll hashval=0;
        set<ll>S;
        for(int j=div[i];j<=s.size();j+=div[i]){
            Hash=(hashvalue[j]-hashvalue[hashval]+mod)%mod;
            Hash=(Hash%mod*moduloinverse(power[hashval]%mod))%mod;
            hashval=j;
            S.insert(Hash);
        }
        if(S.size()==1){
...
int main ()
{
    ...
    vector<int>divs1=divisor(m);
    prehash_calc(s);
    divofstring(s,divs);

    prehash_calc(s1);
    divofstring(s1,divs1);
    ans=Counter-M.size();
    cout<<ans<<endl;
