...
int call (int i, int j){
	...
	//cout<<i<<" " <<j<<" exit 1"<<endl;
	//cout<<sum<<" sum  1g"<<endl;
	if(M[{i,i+1}]!=1){
		//cout<<" entered 2"<<endl;
		if(!vis[{i-n,i}])sum += call(i - n, i) + 1; //left
	}
	//cout<<i<<" " <<j<<" exit 2"<<endl;
	//cout<<sum<<" sum  2g"<<endl;
	if(M[{j,j+1}]!=1){
		//cout<<" entered 3"<<endl;
		if(!vis[{j,j+n}])sum += call(j, j + n) + 1; //right
	}
	//cout<<i<<" " <<j<<" exit 3"<<endl;
	//cout<<sum<<" sum 3 g"<<endl;
	if(M[{i+1,j+1}]!=1){
		//cout<<" entered 4"<<endl;
		if(!vis[{i+1,j+1}])sum += call(i + 1, j + 1) + 1; //down
	}
	//cout<<i<<" " <<j<<" exit 4"<<endl;
	
	//cout<<sum<<" sum  4g"<<endl;
	return sum;
}
int main ()
{
